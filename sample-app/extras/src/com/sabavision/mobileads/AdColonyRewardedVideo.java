...
import com.sabavision.common.util.Json;
...
public class AdColonyRewardedVideo extends CustomEventRewardedVideo {
     ...
     * in the serverExtras Map by specifying Custom Event Data in SabaVision's web interface.
     *
    ...
    public void showVideo() {
        if (this.hasVideoAvailable()) {
            ...
            SabaVisionRewardedVideoManager.onRewardedVideoPlaybackError(AdColonyRewardedVideo.class, mZoneId, SabaVisionErrorCode.VIDEO_PLAYBACK_ERROR);
        }
    }
    ...
    private void setUpGlobalSettings() {
        final AdColonyGlobalMediationSettings globalMediationSettings =
                SabaVisionRewardedVideoManager.getGlobalMediationSettings(AdColonyGlobalMediationSettings.class);
    ...
    private boolean getConfirmationDialogFromSettings() {
        final AdColonyInstanceMediationSettings settings =
                SabaVisionRewardedVideoManager.getInstanceMediationSettings(AdColonyInstanceMediationSettings.class, mAdUnitId);
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (mAd.getAvailableViews() > 0) {
                                SabaVisionRewardedVideoManager.onRewardedVideoLoadSuccess(
                                        AdColonyRewardedVideo.class,
                                        mZoneId);
                            } else {
                                SabaVisionRewardedVideoManager.onRewardedVideoLoadFailure(
                                        AdColonyRewardedVideo.class,
                                        mZoneId,
                                        SabaVisionErrorCode.NETWORK_NO_FILL);
                            }
                        }
                    });
                }
    ...
    private static class AdColonyListener implements AdColonyAdListener,
        ...
        public void onAdColonyAdAttemptFinished(final AdColonyAd adColonyAd) {
            ...
            SabaVisionRewardedVideoManager.onRewardedVideoClosed(AdColonyRewardedVideo.class, zoneId);
            if (adColonyAd.notShown()) {
                if (adColonyAd.canceled() || adColonyAd.skipped()) {
                    SabaVisionLog.d("User canceled ad playback");
                    return;
                }

                SabaVisionErrorCode reason = SabaVisionErrorCode.VIDEO_DOWNLOAD_ERROR;
                if (adColonyAd.noFill()) {
                    reason = SabaVisionErrorCode.NETWORK_NO_FILL;
                }
        ...
        public void onAdColonyAdStarted(final com.jirbo.adcolony.AdColonyAd adColonyAd) {
            SabaVisionRewardedVideoManager.onRewardedVideoStarted(
                    AdColonyRewardedVideo.class,
        ...
        public void onAdColonyV4VCReward(final AdColonyV4VCReward adColonyV4VCReward) {
            SabaVisionReward reward;
            if (adColonyV4VCReward.success()) {
                reward = SabaVisionReward.success(adColonyV4VCReward.name(), adColonyV4VCReward.amount());
            } else {
                reward = SabaVisionReward.failure();
            }
