...
class MillennialBanner extends CustomEventBanner {
    ...
    protected void loadBanner(final Context context, final CustomEventBannerListener customEventBannerListener,
        ...
        if (!initializeSDK(context)) {
            ...
            UI_THREAD_HANDLER.post(new Runnable() {
                @Override
                public void run() {
                    mBannerListener.onBannerFailed(SabaVisionErrorCode.INTERNAL_ERROR);
                }
            });
            return;
        }
        ...
        if (extrasAreValid(serverExtras)) {
            ...
            UI_THREAD_HANDLER.post(new Runnable() {
                @Override
                public void run() {
                    mBannerListener.onBannerFailed(SabaVisionErrorCode.ADAPTER_CONFIGURATION_ERROR);
                }
            });
            return;
        }
        ...
        try {
        /* If SabaVision gets location, so do we. */
            ...
            if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                if (!MMSDK.isInitialized()) {
                    if (context instanceof Activity) {
                        ...
                        Log.e(TAG, "MMSDK.initialize must be explicitly called when instantiating the SabaVision AdView or InterstitialAd without an Activity.");
                        return false;
                    }
                }
            } else {
    ...
    class MillennialInlineListener implements InlineAd.InlineListener {
        ...
        public void onAdLeftApplication(InlineAd inlineAd) {
            // Intentionally not calling SabaVision's onLeaveApplication to avoid double-count
        ...
        public void onRequestFailed(InlineAd inlineAd, InlineErrorStatus inlineErrorStatus) {
            ...
            SabaVisionErrorCode sabavisionErrorCode;

            switch (inlineErrorStatus.getErrorCode()) {
                case InlineErrorStatus.ADAPTER_NOT_FOUND:
                    sabavisionErrorCode = SabaVisionErrorCode.ADAPTER_CONFIGURATION_ERROR;
                    break;
                case InlineErrorStatus.DISPLAY_FAILED:
                    sabavisionErrorCode = SabaVisionErrorCode.INTERNAL_ERROR;
                    break;
                case InlineErrorStatus.INIT_FAILED:
                    sabavisionErrorCode = SabaVisionErrorCode.WARMUP;
                    break;
                case InlineErrorStatus.NO_NETWORK:
                    sabavisionErrorCode = SabaVisionErrorCode.NO_CONNECTION;
                    break;
                case InlineErrorStatus.UNKNOWN:
                    sabavisionErrorCode = SabaVisionErrorCode.UNSPECIFIED;
                    break;
                case InlineErrorStatus.LOAD_FAILED:
                default:
                    sabavisionErrorCode = SabaVisionErrorCode.NETWORK_NO_FILL;
            }

            final SabaVisionErrorCode fErrorCode = sabavisionErrorCode;
            UI_THREAD_HANDLER.post(new Runnable() {
                @Override
                public void run() {
