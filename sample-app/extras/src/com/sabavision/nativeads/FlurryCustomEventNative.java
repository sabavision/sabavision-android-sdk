...
import com.sabavision.mobileads.FlurryAgentWrapper;
...
public final class FlurryCustomEventNative extends CustomEventNative {
    ...
    /**
     * Key for enabling Flurry debug logging. After manually creating a {@link SabaVisionNative} object,
     * pass in <code>true</code> as a value for this key in the map passed to
     * {@link SabaVisionNative#setLocalExtras(Map)}.
     *
     * E.g.
     *
     * <blockquote><pre>
     ...
     * mSabaVisionNative.setLocalExtras(adapterExtras);
     * }
     * </pre></blockquote>
     *
    ...
    private static final double SABAVISION_STAR_RATING_SCALE = StaticNativeAd.MAX_STAR_RATING;
    ...
    private static synchronized void mapNativeAd(@NonNull final FlurryBaseNativeAd sabavisionSupportedAd,
        ...
        if (coverImageAsset != null && !TextUtils.isEmpty(coverImageAsset.getValue())) {
            sabavisionSupportedAd.setMainImageUrl(coverImageAsset.getValue());
        }
        if (iconImageAsset != null && !TextUtils.isEmpty(iconImageAsset.getValue())) {
            sabavisionSupportedAd.setIconImageUrl(iconImageAsset.getValue());
        }

        sabavisionSupportedAd.setTitle(flurryAdNative.getAsset(ASSET_HEADLINE).getValue());
        sabavisionSupportedAd.setText(flurryAdNative.getAsset(ASSET_SUMMARY).getValue());
        sabavisionSupportedAd.addExtra(EXTRA_SEC_BRANDING_LOGO,
                flurryAdNative.getAsset(ASSET_SEC_HQ_BRANDING_LOGO).getValue());

        if (sabavisionSupportedAd.isAppInstallAd()) {
            ...
            if (ratingHqImageAsset != null && !TextUtils.isEmpty(ratingHqImageAsset.getValue())) {
                sabavisionSupportedAd.addExtra(EXTRA_STAR_RATING_IMG, ratingHqImageAsset.getValue());
            } else {
                ...
                if (ratingImageAsset != null && !TextUtils.isEmpty(ratingImageAsset.getValue())) {
                    sabavisionSupportedAd.addExtra(EXTRA_STAR_RATING_IMG, ratingImageAsset.getValue());
                }
            }

            FlurryAdNativeAsset appCategoryAsset = flurryAdNative.getAsset(ASSET_APP_CATEGORY);
            if (appCategoryAsset != null) {
                sabavisionSupportedAd.addExtra(EXTRA_APP_CATEGORY, appCategoryAsset.getValue());
            }
            FlurryAdNativeAsset appRatingAsset = flurryAdNative.getAsset(ASSET_APP_RATING);
            if (appRatingAsset != null) {
                sabavisionSupportedAd.setStarRating(getStarRatingValue(appRatingAsset.getValue()));
            }
        }

        FlurryAdNativeAsset ctaAsset = flurryAdNative.getAsset(ASSET_CALL_TO_ACTION);
        if (ctaAsset != null) {
            sabavisionSupportedAd.setCallToAction(ctaAsset.getValue());
        }

        if (sabavisionSupportedAd.getImageUrls().isEmpty()) {
            Log.d(LOG_TAG, "preCacheImages: No images to cache for Flurry Native Ad: " +
                    flurryAdNative.toString());
            sabavisionSupportedAd.onNativeAdLoaded();
        } else {
            sabavisionSupportedAd.precacheImages();
        }
    }
        ...
        if (appRatingString != null) {
            ...
            if (ratingParts.length == 2) {
                try {
                    ...
                    rating = (numer / denom) * SABAVISION_STAR_RATING_SCALE;
    ...
    private boolean shouldAllowVideoNativeAds() {
        try {
            Class.forName("com.sabavision.nativeads.FlurryNativeAdRenderer");
     ...
     * {@link SabaVisionStaticNativeAdRenderer}.
     *
